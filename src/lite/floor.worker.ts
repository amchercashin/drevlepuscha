...
import {computeNormals} from './geometry.ts';
export type PackedFloor={positions:Float32Array;indices:Uint32Array;colors:Float32Array;uvs:Float32Array;normals:Float32Array};
let boxes:Box[]=[];enableShowcase();
function pack(g:FloorGeometry):PackedFloor{
 const normals=computeNormals(g.positions,g.indices);
 ...
 return {positions:new Float32Array(g.positions),indices:new Uint32Array(g.indices),colors:new Float32Array(g.colors),uvs:new Float32Array(g.uvs),normals};
}
self.onmessage=({data})=>{
 ...
 try{
  ...
  const transfer=Object.values(result).flatMap(g=>Object.values(g).map(a=>a.buffer)) as Transferable[];
  (self as unknown as {postMessage:(m:unknown,o:{transfer:Transferable[]})=>void}).postMessage({job:data,data:result,buildMs:performance.now()-start},{transfer});
