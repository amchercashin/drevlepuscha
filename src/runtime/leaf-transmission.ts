...
export class LeafTransmission extends MaterialPluginBase {
 ...
 override getUniforms(language:ShaderLanguage){return {ubo:[{name:'leafSunDirection',size:3,type:'vec3'},{name:'leafSunColor',size:3,type:'vec3'}],fragment:language===0?'#ifndef UNIFORMBUFFERS\nuniform vec3 leafSunDirection;\nuniform vec3 leafSunColor;\n#endif':''};}
 ...
 override getCustomCode(type:string,language:ShaderLanguage){
  ...
  return {CUSTOM_FRAGMENT_BEFORE_FOG:wgsl?`
  ...
  `:`
   float foliage=smoothstep(0.025,0.11,baseColor.g-baseColor.r);
   float forwardScatter=pow(max(0.0,dot(-viewDirectionW,leafSunDirection)),5.0);
   float thinEdge=pow(1.0-abs(dot(normalW,viewDirectionW)),1.5);
   float backlit=0.25+0.75*max(0.0,dot(-normalW,leafSunDirection));
   float transmitted=foliage*forwardScatter*backlit*(0.12+0.55*thinEdge);
   color.rgb+=vec3(0.68,0.86,0.24)*leafSunColor*transmitted;
  `};
 }
}
