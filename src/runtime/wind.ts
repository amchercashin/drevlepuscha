...
import type {WindPreset,WindSnapshot} from '../domain/wind.ts';
import presets from '../../config/wind-presets.json';
import type {Point3} from '../domain/harness.ts';

export type WindSettings={enabled:boolean;intensity:number;preset:WindPreset};
export const DEFAULT_WIND:WindSettings={enabled:true,intensity:1,preset:'forest'};
...
export class WindSystem {
 ...
 configure(settings:Partial<WindSettings>){
  ...
  if(typeof settings.enabled==='boolean')this.settings.enabled=settings.enabled;
  if(Number.isFinite(settings.intensity))this.settings.intensity=Math.max(0,Math.min(1.5,settings.intensity!));
  if(settings.preset==='forest'||settings.preset==='enchanted')this.settings.preset=settings.preset;
  this.apply();
 }
 ...
 private apply(){const s=this.shared?DEFAULT_WIND:this.settings;this.enabled=s.enabled;this.weather.setProfile(presets[s.preset]);this.changed();}
 update(dt:number,eye:Point3){
  const start=performance.now();this.weather.update(dt);Object.assign(this.eye,eye);
  const target=(this.shared?DEFAULT_WIND:this.settings).intensity;
