...
export async function createRenderer(canvas:HTMLCanvasElement):Promise<RuntimeRenderer> {
  ...
  if(requested!=='webgl2'){
    ...
    try {
      ...
      const gpu=new WebGPUEngine(canvas,{antialias:true,enableAllFeatures:false,setMaximumLimits:false});candidate=gpu;
      await gpu.initAsync();
