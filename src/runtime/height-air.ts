...
void main(){
 ...
 gl_FragColor=vec4(sunColor*(1.0-exp(-optical))*0.28*phase,exp(-optical*0.22));
}`;
...
fn main(input: FragmentInputs)->FragmentOutputs {
 ...
 fragmentOutputs.color=vec4f(uniforms.sunColor*(1.0-exp(-optical))*0.28*phase,exp(-optical*0.22));
}`;
