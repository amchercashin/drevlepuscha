...
export class LodDither extends MaterialPluginBase {
 constructor(material:Material){super(material,'LodDither',200,{LOD_DITHER:false},true,false);this.registerForExtraEvents=true;this._enable(true);}
 override prepareDefines(defines:MaterialDefines,_scene:Scene,mesh:AbstractMesh){(defines as MaterialDefines & {LOD_DITHER:boolean}).LOD_DITHER=mesh.name.endsWith('-fade');}
 override isCompatible(language: ShaderLanguage) { return language === ShaderLanguage.WGSL; }
 override getUniforms(){return {ubo:[{name:'lodCoverage',size:2,type:'vec2'}]};}
