...
export class CanopyShade extends MaterialPluginBase {
 ...
 override getUniforms(language:ShaderLanguage){return {ubo:[{name:'canopySunMatrix',size:16,type:'mat4'}],fragment:language===0?'#ifndef UNIFORMBUFFERS\nuniform mat4 canopySunMatrix;\n#endif':''};}
 ...
 override getCustomCode(type:string,language:ShaderLanguage){
  ...
  return {
   ...
   `:`
    vec3 canopyPosition=vPositionW;
    float canopyDensity=0.68+0.20*sin(canopyPosition.x*0.31+canopyPosition.z*0.19)*sin(canopyPosition.z*0.27-canopyPosition.x*0.13);
    float terrain=3.2*smoothstep(32.0,47.0,-canopyPosition.z)+0.18*sin(canopyPosition.x*0.2)*sin(-canopyPosition.z*0.14);
    float canopyHeight=canopyPosition.y-terrain;
    float canopyVisibility=1.0-0.72*canopyDensity*(1.0-smoothstep(5.0,14.0,canopyHeight));
    vec4 canopyProjected=canopySunMatrix*vec4(canopyPosition,1.0);
    float canopyEdge=max(abs(canopyProjected.x/canopyProjected.w),abs(canopyProjected.y/canopyProjected.w));
    float canopyBlend=smoothstep(0.55,0.90,canopyEdge);
   `,
