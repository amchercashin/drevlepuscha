...
export function createBirdAnimator(groups:AnimationGroup[],takeoffMs=400,art?:import('./assets.ts').AnimalArt){
 ...
 return {
  update(pose:WildlifePose,simMs:number,hz:number){
   const names:Record<string,string>={perched:'perch_idle',alert:'alert',takeoff:'takeoff',flying:'fly_loop',forage:'forage_idle','ground-bound':'bound_ground',mount:'mount_trunk',climb:'climb_up','trunk-idle':'trunk_idle',graze:'graze_idle','walk-away':'walk_loop',flee:'run_loop',recover:'graze_idle'};const name=names[pose.state],group=clips.get(name);if(!group)return;
   if(name!==previous){old=new Map();const blend=!!previous&&(name==='alert'||name==='perch_idle'||art?.id==='roe-deer');if(blend)for(const {target,animation} of group.targetedAnimations){const value=target[animation.targetProperty];if(value?.clone){if(!old.has(target))old.set(target,new Map());old.get(target)!.set(animation.targetProperty,value.clone());}}switchedAt=blend?simMs:-Infinity;previous=name;lastSample=-Infinity;}
   ...
   const elapsed=Math.max(0,(quantized-(name==='fly_loop'?pose.routeStartMs+takeoffMs:pose.stateSinceMs))/1000),loop=art?.clips[name]?.loop??(name==='perch_idle'||name==='fly_loop');
   const fps=group.targetedAnimations[0]?.animation.framePerSecond??60,span=group.to-group.from;
   const at=group.from+(loop?((elapsed+(['perch_idle','forage_idle','trunk_idle','graze_idle'].includes(name)?pose.animationVariant*.4:0))*fps)%span:Math.min(span,elapsed*fps)),blend=Math.min(1,Math.max(0,(simMs-switchedAt)/120));
