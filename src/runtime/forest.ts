...
export async function createForest(scene:Scene){
 ...
 const templates=data.levels.map((parts,level)=>parts.map((p,i)=>{const m=new Mesh(`source-${level}-${p.name}`,scene);m.sideOrientation=1;const v=new VertexData();Object.assign(v,p);v.applyToMesh(m);m.material=level>=(data.bakedColorFromLevel??Infinity)?bakedMaterials[i]:materials[i];m.setEnabled(false);return m;}));
 ...
 function transform(m:Mesh|InstancedMesh,t:TreePlacement){m.position.set(t.e,t.y,-t.n);m.scaling.set(t.width,t.height,t.depth);m.rotation.set(t.leanX,t.yaw,t.leanZ);m.freezeWorldMatrix();m.isPickable=false;}
 ...
 return {boxes:placements.map(p=>treeCollider(p,data.trunkRadius)),update,stats:()=>({version:FOREST_VERSION,asset:selected?.id??'game',assetLabel:selected?.label,trees:placements.length,activeTrees:active.size,trianglesPerLevel:data.triangles,materialsPerTree:materials.length,lodCounts:[0,1,2].map(l=>[...active.values()].filter(t=>t.level===l).length),lockNear,transitions:[...active.values()].filter(t=>t.previous>=0).length,geometryBuffers:templates.length*materials.length+horizon.stats().cells*materials.length,horizon:horizon.stats()}),setNearOnly:(v:boolean)=>{lockNear=v;forceSwitch=true;},get meshes(){return [...active.values()].flatMap(t=>[...t.fades.values()].flat());}};
}
