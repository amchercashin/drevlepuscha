...
export class SoilPattern extends MaterialPluginBase {
 constructor(material:Material){super(material,'SoilPattern',215,{},true,false);this._enable(true);}
 ...
 override getCustomCode(type:string,language:ShaderLanguage){if(type!=='fragment')return null;
  return {CUSTOM_FRAGMENT_UPDATE_DIFFUSE:language===1?`
   ...
   let edge=abs(e-sin(n*0.075)*1.3)+0.12*sin(n*3.1+e*1.5);
  ...
  `:`
   ...
   float edge=abs(e-sin(n*0.075)*1.3)+0.12*sin(n*3.1+e*1.5);
