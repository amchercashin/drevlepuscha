...
export class SoilPattern extends MaterialPluginBase {
 ...
 override getCustomCode(type:string,language:ShaderLanguage){if(type!=='fragment')return null;
  ...
  `:`
   ...
   vec3 soil=mix(vec3(${showcaseEnabled?'0.22,0.34,0.29':'0.39,0.40,0.30'}),vec3(${showcaseEnabled?'0.20,0.40,0.32':'0.32,0.46,0.34'}),moss);
   vec3 colour=mix(soil,vec3(${showcaseEnabled?'0.64,0.56,0.39':'0.65,0.60,0.44'}),path);
