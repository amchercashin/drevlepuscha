...
try {
  ...
  parts.forEach(p=>{
    ...
    const m=new StandardMaterial(p.name,scene);m.diffuseColor=Color3.White();m.specularColor=Color3.Black();m.ambientColor=new Color3(0.15,0.15,0.15);if(p.name==='bark')m.diffuseTexture=barkTexture;if(p.name==='canopy')m.diffuseTexture=canopyTexture;mesh.material=m;mesh.freezeWorldMatrix();meshes.push(mesh);
  });
  ...
  function preset(name:string){const p=presets[name];if(!p)return;camera.inertialAlphaOffset=camera.inertialBetaOffset=camera.inertialRadiusOffset=0;camera.inertialPanningX=camera.inertialPanningY=0;camera.setTarget(p.target);camera.setPosition(p.position);if(['front','right','back','left'].includes(name))camera.radius=Math.max(camera.radius,14/(2*Math.tan(camera.fov/2)*(canvas.clientWidth/canvas.clientHeight)));document.querySelectorAll<HTMLButtonElement>('[data-view]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.view===name)));}
