...
import {FrameWorkBudget,SceneStartup,bindInputFocus} from '../runtime/startup.ts';
...
import {createScoutActors} from './scout-actors.ts';
import {createHuntInterface} from './hunt-interface.ts';
import {createRegionWeapons} from './weapons.ts';
...
scene.clearColor=new Color4(.74,.8,.84,1);scene.fogMode=Scene.FOGMODE_EXP2;scene.fogDensity=.00028;scene.fogColor=new Color3(.76,.82,.84);
...
const weapons=createRegionWeapons(scene,hero);
...
const hunt=new RegionHunt(world.data.geo.source,world.data.manifest.version),huntSaveKey='region:brandywine-bridge:hunt-v1';
try{const stored=localStorage.getItem(huntSaveKey);if(stored)hunt.restore(JSON.parse(stored));}catch(e){console.warn('Сохранение дозора не загружено:',e);}
...
world.setWater(player.water);const undergrowth=createUndergrowth(scene,world),meadow=createMeadow(world);
const scoutActors=createScoutActors(scene,world,hunt);
...
const performanceSamples:number[]=[],renderSamples:number[]=[],cpuSamples:number[]=[];let saveAt=0,audioTreesAt=0;
...
const huntWorld:HuntWorld={height:(e,n)=>world.data.height(e,n),visible:(a,b)=>regionSight({e:a.e,n:a.n,h:a.height},{e:b.e,n:b.n,h:b.height},world.data.height.bind(world.data)).visible,blocked:(e,n)=>query.blocked(e,n)||query.waterDepth(e,n)>.35};
function state(){const sorted=[...performanceSamples].sort((a,b)=>a-b),render=[...renderSamples].sort((a,b)=>a-b),cpu=[...cpuSamples].sort((a,b)=>a-b);return {ready:start.readyAt!==null,paused,loading,frames,player:{...player},camera:{yaw,pitch,distance},world:world.stats(),wildlife:wildlife.stats(),actors:actors.stats(),scouts:scoutActors.stats(),hunt:hunt.snapshot(),rain:rain.stats(),audio:audio.stats(),wind:wind.stats(),lighting:daylight.stats(),errors:[...errors,...structures.failures,...world.assets.failures],renderer:renderer.kind,gpu:renderer.info,frameMs:{samples:sorted.length,median:sorted[Math.floor(sorted.length*.5)],p95:sorted[Math.floor(sorted.length*.95)]},renderCpuMs:{median:render[Math.floor(render.length*.5)],p95:render[Math.floor(render.length*.95)]},frameCpuMs:{median:cpu[Math.floor(cpu.length*.5)],p95:cpu[Math.floor(cpu.length*.95)]},resolution:[engine.getRenderWidth(),engine.getRenderHeight()],gateOpen:session.state.gateOpen,boats:session.state.boats,encounter:session.state.encounter};}
function save(){try{if(restoreNotice){ui.show('Предыдущее сохранение несовместимо. Новая прогулка сохраняется отдельно.');localStorage.setItem(saveKey+':previous',localStorage.getItem(saveKey)??'');restoreNotice='';}localStorage.setItem(saveKey,JSON.stringify(session.snapshot()));localStorage.setItem(huntSaveKey,JSON.stringify(hunt.snapshot()));localStorage.setItem('region:brandywine-bridge:camera',JSON.stringify({yaw,pitch,distance}));}catch{ui.show('Браузер не смог сохранить прогулку.');}}
...
const ui=regionInterface(session,world,hunt,{interact,resolve(choice){if(session.resolve(choice)){ui.show(session.state.encounter.outcome!);save();}},water(value:WaterState){if(session.setWater(value)){world.setWater(value);save();}else ui.show('Уровень воды сейчас менять нельзя: выйдите на сухую землю.');},save(){save();ui.show('Прогулка сохранена.');},pause:setPaused});
...
const bowElevation=()=>Math.max(-.12,Math.min(.35,.04-(pitch-6)*Math.PI/300));
function attack(){if(paused||loading||document.querySelector('dialog[open]'))return;const body={e:player.e,n:player.n,height:player.height,mode:player.mode,speed:0,crouching:keys.has('ControlLeft')||keys.has('ControlRight')};
 const fired=hunt.state.weapon==='bow'?hunt.fireBow(body,yaw,bowElevation()):hunt.swing(body,yaw,huntWorld);
 if(!fired&&hunt.state.weapon==='bow'&&hunt.state.arrows===0)ui.show('Колчан пуст. Возьмите стрелы у постоялого двора.');}
...
function interact(){if(paused||loading)return;const action=nearbyAction();if(!action)return;
 ...
 else if(action.id==='hunt-track'){hunt.inspect(player);for(const message of hunt.drainEvents())ui.show(message);}
 else if(action.id==='arrows'){hunt.resupply(player);for(const message of hunt.drainEvents())ui.show(message);}
...
canvas.setAttribute('aria-label','Брендивинский мост. WASD — движение, E — взаимодействие, M — карта, 1 — лук, 2 — меч, левая кнопка мыши — атака, правая — осмотр.');document.querySelector('.eyebrow')!.textContent='Древлепуща / Брендивинский мост';
window.addEventListener('keydown',e=>{if((e.target as HTMLElement)?.matches('input,select,textarea')||document.querySelector('dialog[open]'))return;if(e.code==='KeyM'){ui.toggle();return;}if(e.code==='KeyE'&&!e.repeat){interact();return;}if(e.code==='Digit1'){hunt.select('bow');return;}if(e.code==='Digit2'){hunt.select('sword');return;}if(e.code==='Escape'){setPaused(!paused);return;}if(e.code==='Space'){yaw=player.heading;e.preventDefault();}keys.add(e.code);});window.addEventListener('keyup',e=>keys.delete(e.code));bindInputFocus(canvas,()=>{keys.clear();dragging=false;});
canvas.addEventListener('contextmenu',e=>e.preventDefault());canvas.addEventListener('pointerdown',e=>{if(e.button===0&&e.pointerType==='mouse')attack();if(e.button===2){dragging=true;canvas.setPointerCapture(e.pointerId);}});canvas.addEventListener('pointerup',()=>dragging=false);canvas.addEventListener('pointermove',e=>{if(dragging){yaw=normalizeAzimuth(yaw+e.movementX*.2);pitch=Math.max(-70,Math.min(cameraConfig.travel.pitchMaxDeg,pitch+e.movementY*.16));}});canvas.addEventListener('wheel',e=>{distance=Math.max(cameraConfig.travel.distanceMinM,Math.min(cameraConfig.travel.distanceMaxM,distance+e.deltaY*.004));e.preventDefault();},{passive:false});window.addEventListener('resize',()=>engine.resize());
...
if(new URLSearchParams(location.search).get('debug')==='1')(window as any).__region={state,travel,pause:setPaused,world,player,scene,query,session,hunt,attack,interact,save,ui,daylight,sight:(e:number,n:number)=>regionSight({...player,h:player.height+1.65},{e,n,h:world.data.geo.height(e,n)+1.65},world.data.geo.height)};
window.addEventListener('pagehide',save);scene.onDisposeObservable.add(()=>{audio.dispose();rain.dispose();ranger.dispose();ui.dispose();huntUi.dispose();});
engine.runRenderLoop(()=>{
 if(document.hidden){previous=performance.now();return;}const cpuStart=performance.now(),now=performance.now(),elapsed=now-previous,dt=Math.min(.05,elapsed/1000);previous=now;if(!paused&&!loading){performanceSamples.push(elapsed);if(performanceSamples.length>600)performanceSamples.shift();}
 try{
  let speed=0;const running=keys.has('ShiftLeft')||keys.has('ShiftRight')||touch.state.running;
  if(!paused&&!loading){let forward=Number(keys.has('KeyW'))-Number(keys.has('KeyS'))+touch.state.forward,right=Number(keys.has('KeyD'))-Number(keys.has('KeyA'))+touch.state.right;const len=Math.max(1,Math.hypot(forward,right));forward/=len;right/=len;const a=yaw*Math.PI/180,crouching=keys.has('ControlLeft')||keys.has('ControlRight'),v=crouching?0.95:running?15:1.85,de=(Math.sin(a)*forward+Math.cos(a)*right)*v*dt,dn=(Math.cos(a)*forward-Math.sin(a)*right)*v*dt;
  ...
  const shoulder=hunt.state.weapon==='bow'&&!paused?1.15:0,rightE=Math.cos(yaw*Math.PI/180),rightN=-Math.sin(yaw*Math.PI/180);
  eye.e+=rightE*shoulder;eye.n+=rightN*shoulder;
  ...
  camera.position.set(eye.e-world.origin.e,eye.h,world.origin.n-eye.n);camera.setTarget(new Vector3(hero.position.x+rightE*shoulder,h+.95+Math.tan(Math.max(0,-pitch)*Math.PI/180)*distance,hero.position.z-rightN*shoulder));
  ...
  weapons.update(hunt.state.weapon);
  if(active){const result=hunt.update(active,{e:player.e,n:player.n,height:h,mode:player.mode,speed,crouching:keys.has('ControlLeft')||keys.has('ControlRight')},huntWorld);if(result==='respawn')void travel(-220,-105).catch(e=>errors.push(String(e)));for(const message of hunt.drainEvents())ui.show(message);}
  structures.update(player,h,camera.position,dt,session.state.gateOpen,budget);actors.update(active,budget);scoutActors.update(active,player,new FrameWorkBudget(2,1));undergrowth.update(player,budget);meadow.update(player,budget);wildlife.update(active,{id:'ranger',...player,h,speedMps:speed,headingDeg:player.heading,running,observedAtMs:session.state.clock*1000},{totalGameHours:daylight.stats().totalGameHours,daylight01:daylight.daylightAmount(),precipitation01:daylight.precipitation()},budget);
  const aimElevation=bowElevation(),aimDistance=28,aimSeconds=aimDistance/(52*Math.cos(aimElevation));
  const aim=new Vector3(hero.position.x+rightE*.6+Math.sin(yaw*Math.PI/180)*aimDistance,h+1.45+52*Math.sin(aimElevation)*aimSeconds-4.9*aimSeconds*aimSeconds,hero.position.z-rightN*.6-Math.cos(yaw*Math.PI/180)*aimDistance);
  const projected=Vector3.Project(aim,Matrix.IdentityReadOnly,scene.getTransformMatrix(),camera.viewport.toGlobal(engine.getRenderWidth(),engine.getRenderHeight())),rect=canvas.getBoundingClientRect();
  huntUi.setReticle(rect.left+projected.x/engine.getRenderWidth()*rect.width,rect.top+projected.y/engine.getRenderHeight()*rect.height);
  sun.position.copyFrom(hero.position.subtract(sun.direction.scale(60)));shadows.getShadowMap()!.renderList=[...hero.getChildMeshes(),...world.trees.shadows(player),...actors.shadowMeshes(),...scoutActors.shadowMeshes(),...wildlife.shadowMeshes({...player,h}),...structures.meshes.filter(m=>m.isEnabled()&&Vector3.Distance(m.position,hero.position)<45)];const renderStart=performance.now();scene.render();if(!paused&&!loading){renderSamples.push(performance.now()-renderStart);if(renderSamples.length>600)renderSamples.shift();}frames++;
  ...
  if(now-lastUi>500){lastUi=now;if(!paused&&!loading)session.discover((e,n)=>regionSight({...player,h:player.height+1.65},{e,n,h:world.data.geo.surfaceHeight(e,n)+1},world.data.geo.surfaceHeight).visible);ui.update(nearbyAction()?.label??'');huntUi.update(player,paused,keys.has('ControlLeft')||keys.has('ControlRight'));$('fps').textContent=Math.round(engine.getFps())+' FPS';$('metrics').textContent=`${world.terrain.patches.size} участков · ${world.trees.resident.length} деревьев рядом · кадр p95 ${state().frameMs.p95?.toFixed(1)??'—'} мс`;$('location').textContent=world.data.geo.zoneAt(player.e,player.n)?.name??'Пойма';}
