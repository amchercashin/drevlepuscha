...
engine.runRenderLoop(()=>{
 ...
 try{
  ...
  structures.update(player,h,camera.position,dt,session.state.gateOpen,budget);actors.update(active,budget);undergrowth.update(player,budget);meadow.update(player,budget);wildlife.update(active,{id:'ranger',...player,h,speedMps:speed,headingDeg:player.heading,running,observedAtMs:session.state.clock*1000},{totalGameHours:daylight.stats().totalGameHours,daylight01:daylight.daylightAmount(),precipitation01:daylight.precipitation()},budget);
