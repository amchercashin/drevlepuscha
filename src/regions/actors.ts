...
export function createRegionalActors(scene:Scene,world:RegionWorld,session:RegionSession){
 ...
 const boatModels=new Map<string,Mesh[][]>(),assetJobs:(()=>void)[]=[];
 const moorings:Mesh[]=[];
 ...
 void world.assets.load('rowboat').then(levels=>assetJobs.push(()=>{for(const [id,root]of boats){for(const m of root.getChildMeshes())m.setEnabled(false);
  boatModels.set(id,levels.map((parts,li)=>parts.map((source,i)=>{const m=new Mesh(id+'-meshy-'+li+'-'+i,scene);source.geometry!.applyToMesh(m);m.material=source.material;m.sideOrientation=1;m.parent=root;m.position.y=-.23;m.isPickable=false;m.receiveShadows=true;m.setEnabled(false);return m;})));
 }})).catch(()=>{});
 ...
 return {
  update(dt:number,budget:FrameWorkBudget){if(assetJobs.length)budget.run(()=>assetJobs.shift()!());const player=session.state.player;for(const b of session.state.boats){const root=boats.get(b.id)!,distance=Math.hypot(player.e-b.e,player.n-b.n);root.position.set(b.e-world.origin.e,(world.data.geo.waterAt(b.e,b.n,player.water)?.level??0)+.06,world.origin.n-b.n);root.rotation.y=-b.heading*Math.PI/180;root.setEnabled(distance<500);for(const [li,meshes]of (boatModels.get(b.id)??[]).entries())for(const m of meshes)m.setEnabled(li===(distance<50?0:distance<150?1:2));}
