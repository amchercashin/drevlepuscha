...
function state(){const sorted=[...samples].sort((a,b)=>a-b),render=[...renderSamples].sort((a,b)=>a-b),cpu=[...cpuSamples].sort((a,b)=>a-b);return {ready:startup.readyAt!==null,paused,loading,frames,player:{...player},hunt:hunt.snapshot(),visitor:visitors.visitor,world:data.stats(),graphics:renderer.stats(),renderer:'direct-webgpu',frameMs:{samples:sorted.length,median:sorted[Math.floor(sorted.length*.5)],p95:sorted[Math.floor(sorted.length*.95)]},renderCpuMs:{median:render[Math.floor(render.length*.5)],p95:render[Math.floor(render.length*.95)]},frameCpuMs:{median:cpu[Math.floor(cpu.length*.5)],p95:cpu[Math.floor(cpu.length*.95)]},errors};}
