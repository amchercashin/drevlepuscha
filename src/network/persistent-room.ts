...
export class PersistentRoom implements WalkSession {
 ...
 private room:Room;private moves:MessageAction<Position>;private serverPeer:string|null=null;
 ...
 constructor(readonly invite:PersistentInvitation,name:string,private options:RoomOptions={}){
  ...
  this.room=joinRoom({appId:ROOM_APP,password:invite.key,passive:true,relayConfig:{urls:RELAYS},rtcConfig:{iceServers:ICE_SERVERS}},invite.room,{
   ...
   onPeerHandshake:async(peer,send,receive)=>{
    ...
    await send({v:ROOM_VERSION,room:invite.room,session,name:cleanName(name),nonce});
  ...
  states.onMessage=(data,{peerId})=>{if(!this.stopped&&peerId===this.serverPeer&&validSnapshot(data)&&data.tick>this.lastTick&&data.players.some(p=>p.id===this.id)&&data.peers[this.id]===selfId)this.accept(data);};
