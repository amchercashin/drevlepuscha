...
export class Horizon {
    ...
    update(p: EN,budget?:FrameWorkBudget) {
        ...
        void this.data.call('groves', { p: point, origin: this.origin }).then(result => {const upload=()=>{ if (generation !== this.generation)
            return; this.mesh.position.setAll(0);this.buffer.set(result.matrices); this.count = result.count; this.mesh.thinInstanceCount = this.count; this.mesh.thinInstanceBufferUpdated('matrix'); this.mesh.setBoundingInfo(new BoundingInfo(new Vector3(point.e - this.origin.e - 1700, -100, this.origin.n - point.n - 1700), new Vector3(point.e - this.origin.e + 1700, 500, this.origin.n - point.n + 1700))); this.mesh.setEnabled(this.count > 0);};if(budget)this.completed=upload;else upload(); }).catch(() => { });
    }
    rebase(origin: EN) {this.mesh.position.x+=this.origin.e-origin.e;this.mesh.position.z+=origin.n-this.origin.n;this.generation++;this.completed=undefined;this.origin = origin; this.last = ''; }
}
