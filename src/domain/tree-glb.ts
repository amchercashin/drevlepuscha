...
export function treeGLB(parts:TreePart[],textures:Partial<Record<string,Uint8Array>>={}):ArrayBuffer {
  ...
  const doc={asset:{version:'2.0',generator:TREE_VERSION},scene:0,scenes:[{nodes:parts.map((_,i)=>i)}],nodes:parts.map((p,i)=>({name:p.name,mesh:i})),meshes,materials:parts.map(p=>({name:p.name,pbrMetallicRoughness:{baseColorFactor:[1,1,1,1],metallicFactor:0,roughnessFactor:1,...(textureIndices[p.name]!==undefined?{baseColorTexture:{index:textureIndices[p.name]}}:{})}})),buffers:[{byteLength:offset}],bufferViews:views,accessors,images,textures:textureDefs,samplers:[{magFilter:9729,minFilter:9987,wrapS:10497,wrapT:10497}],extras:{units:'metres',origin:'root base',source:'Authored interpretation of user-supplied multi-view tree sheet; not photogrammetry',version:TREE_VERSION}};
