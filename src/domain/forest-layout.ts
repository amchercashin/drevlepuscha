...
export interface ForestLayoutOptions {mode:'showcase'|'legacy-m1';rootRadiusM:number;variation?:TreeVariation;heightAt:(e:number,n:number)=>number;}
export function generateForestPlacements({mode,rootRadiusM,variation,heightAt}:ForestLayoutOptions):TreePlacement[]{
 ...
 function tree(id:string,e:number,n:number,width?:number,height?:number){
  ...
  const samples=variation?24:12;let y=heightAt(e,n);for(let i=0;i<samples;i++){const a=i*Math.PI*2/samples;y=Math.min(y,heightAt(e+Math.cos(a)*radius,n+Math.sin(a)*radius));}
