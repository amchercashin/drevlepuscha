...
export function treeFamilySlot(p:Pick<TreePlacement,'id'|'e'|'n'>,counts:readonly [number,number]=[3,3],showcase=showcaseEnabled):number{
 if(['narrow-left','narrow-right','camera-trunk','canopy-probe'].includes(p.id))return 0;
 if(showcase){const r=createRandom(seedFor('ravine-species',p.id)),f=r();return f<.16?0:f<.38?1+Math.floor(r()*counts[0]):1+counts[0]+Math.floor(r()*counts[1]);}
