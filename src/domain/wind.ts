...
export function sampleWind(s:WindSnapshot,x:number,_y:number,z:number):WindSample{
 ...
 return {directionToXZ:s.directionToXZ,strength01:Math.min(1,s.base*(.78+.22*field)+gust),gust01:gust,turbulence01:field*(s.base+gust)};
}
...
export class WindWeather {
 ...
 private step(dt:number){
  ...
  if(this.time>=this.nextGust){this.gustStart=this.time;this.rise=1+this.random()*2;this.peak=.65+this.random()*.35;this.nextGust=this.time+12+this.random()*18;}
  const response=1-Math.exp(-dt/3);
  s.base+=(p.base-s.base)*response;s.scale+=(p.scale-s.scale)*response;
  const desired=p.gust*this.peak*gustEnvelope(this.time-this.gustStart,this.rise,p.decay);
  s.gust+=(desired-s.gust)*(1-Math.exp(-dt/.6));
  s.directionToXZ[0]=Math.cos(this.angle);s.directionToXZ[1]=Math.sin(this.angle);
  const speed=1.2+s.base*2+s.gust*3;
  ...
  s.motionPhases[0]=wrapPhase(s.motionPhases[0]+dt*.72);s.motionPhases[1]=wrapPhase(s.motionPhases[1]+dt*2.1);
 }
