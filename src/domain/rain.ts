...
/** Reserve a second set of columns for heavy rain; ordinary rain keeps its old density. */
export function rainDropIndices(quality:number){
 const base=quality===0?64:128;
 return Array.from({length:base*2},(_,i)=>i<base?i:128+i-base);
}
...
export function rainDrop(tileX:number,tileZ:number,index:number){
 return {x:(tileX+random(tileX,tileZ,index,0))*RAIN_TILE_SIZE,z:(tileZ+random(tileX,tileZ,index,1))*RAIN_TILE_SIZE,
  ...
  threshold:index<128?random(tileX,tileZ,index,5):.55+.37*random(tileX,tileZ,index,5)};
}
