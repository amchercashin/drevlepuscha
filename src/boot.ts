...
if(params.get('engine')==='lite'){
 ...
 if(scene==='region'){if(params.get('region')!=='brandywine-bridge')throw Error('Неизвестная область');document.body.classList.remove('showcase');if(params.get('renderer')==='babylon'&&params.get('debug')==='1')await import('./regions/game.ts');else await import('./native/regions/brandywine-main.ts');}
