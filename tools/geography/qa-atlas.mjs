...
try{
 ...
 await page.locator('#house').click();assert.match(await page.locator('#details').innerText(),/Дом Тома/);await page.locator('[data-local-relief]').waitFor();await page.screenshot({path:resolve(out,'house.png'),fullPage:true});
 ...
 await page.screenshot({path:resolve(out,'typography.png'),fullPage:true});assert.deepEqual(errors,[]);const result={status:'passed',checks:['overview','house zoom','layers','search','SVG click','profile','JSON download','390px layout','no page errors'],fonts,errors};writeFileSync(resolve(out,'qa.json'),JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify(result));
