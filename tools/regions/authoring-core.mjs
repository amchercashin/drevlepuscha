...
export function nearestStation(e, n, stations) {
  ...
  for (let i = 1; i < stations.length; i++) {
    ...
    if (d < best.distance) best = { distance: d, e: x, n: y, segment: i - 1, t, level: a[2] + t * (b[2] - a[2]), width: a[3] + t * (b[3] - a[3]), depth: a[4] + t * (b[4] - a[4]) };
  }
  return best;
}
...
export function previewHeight(source, e, n, detailed = true) {
  ...
  for (const river of source.rivers) {
    ...
    if (q.distance <= half) {
      ...
      const weight = 1 - smooth((q.distance - river.floodplainHalfWidthM) / river.blendM);
...
export function bankProfile(source, river, q, e, n, detailed = true) {
  ...
  if (!detailed) return { heightM, widthM };
...
export function previewWater(source, e, n, state = source.states.defaultWater) {
  ...
  const bodies = source.rivers.map(r => ({ id: r.id, ...nearestStation(e, n, r.stations) })).filter(q => q.distance <= q.width / 2);
  if (!bodies.length) return null;
  const level = Math.max(...bodies.map(q => q.level)) + source.states.waterLevelsM[state];
  return { level, depth: Math.max(0, level - previewHeight(source, e, n)), bodies: bodies.map(q => q.id) };
}
