...
test('shared renderer rebases the same entity and debug routes; budget and disposal are bounded',()=>{
 ...
 try{
  ...
  render.prepare(new FrameWorkBudget(100,1));assert.equal(render.stats().meshes,1);const mesh=scene.getMeshByName(`TEST-bird:${frame.entities[0].id}`),before=mesh.position.clone();
  render.setProbe(true);assert.equal(render.stats().debugRoutes,2);
  ...
  assert.deepEqual(mesh.metadata.absolutePoint,frame.entities[0].point);assert.equal(render.shadowMeshes(frame.entities[0].point).length,1);
