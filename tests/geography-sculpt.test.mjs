...
test('sculpt: macro geography remains 1:1 while local terrain is authored',()=>{
 ...
 assert.ok(Math.abs(Math.abs(area)/2e6-717.903)<1e-6);
 assert.ok(Math.abs(manifest.summary.routeKm-42.99735459191721)<1e-6);
 assert.ok(manifest.summary.walkingHoursAt1_85Mps>6.4&&manifest.summary.walkingHoursAt1_85Mps<6.5);
 assert.equal(manifest.summary.frameWidthKm,40.96);assert.equal(manifest.summary.frameHeightKm,45.056);
 assert.equal(g.contentRevision,'old-forest-1to1-art-2.0.0');assert.equal(g.zones.length,9);
 assert.equal(g.calibration.gameplayPresentation.macroScale,'1:1');
});
