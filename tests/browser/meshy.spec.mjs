...
for(const [route,triangles] of [['a',5318],['b',5470]])test('Meshy '+route+' renders, exports and walks in the shared forest',async({page},info)=>{
 ...
 await page.goto('/?scene=m1&variety=0&tree=meshy-'+route+'&debug=1&renderer='+info.project.name);await page.waitForFunction(()=>window.m0?.state().ready);await page.getByRole('button',{name:'Начать прогулку'}).click();
