...
test('W2 persistent bird: same snapshot, new epoch after restart, same invitation, empty room stops ticking',async({browser})=>{
 ...
 try{
  ...
  const [a,b]=pages;for(const p of pages)await p.waitForFunction(()=>m0.state().multiplayer.players===2&&m0.wildlife.state().frame.entities.length===1);
  ...
  expect((await a.evaluate(()=>m0.wildlife.state().frame.entities[0].route))).toEqual(await b.evaluate(()=>m0.wildlife.state().frame.entities[0].route));
