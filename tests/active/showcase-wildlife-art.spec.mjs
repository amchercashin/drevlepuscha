...
test('W3 art: two clients share the bird, fresh audio once, independent skins and relocated habitat',async({page,browser})=>{
 ...
 try{
  ...
  await page.waitForFunction(()=>m0.wildlife.state().instances.length===1);
  ...
  for(const p of [page,guest]){await p.getByRole('button',{name:'К птице',exact:true}).click();await p.waitForFunction(()=>m0.wildlife.state().instances.some(i=>i.lod===0&&i.clip==='alert'));}
  ...
  await page.getByLabel('Место птицы').selectOption('trail-bird-02');await page.getByRole('button',{name:'Отойти',exact:true}).click();
  await page.waitForFunction(()=>m0.wildlife.state().frame.entities.some(e=>e.siteId==='trail-bird-02'));
  await page.getByRole('button',{name:'К птице',exact:true}).click();await page.waitForFunction(()=>m0.wildlife.state().instances.some(i=>i.id.includes('trail-bird-02')&&i.lod===0));
