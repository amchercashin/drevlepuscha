...
test('W2 solo bird, actual tree/prop catalog parity, quality and disposal',async({page})=>{
 ...
 await page.goto('/?debug=1&wildlife=1&wildlifeTestModel=1');await page.waitForFunction(()=>window.m0?.state().ready);
 ...
 await page.locator('#resume').click();await page.waitForFunction(()=>m0.wildlife.state().frame.entities.some(e=>e.siteId==='trail-bird-01'));
 const initial=await page.evaluate(()=>m0.wildlife.state());expect(initial.testOnly).toBe(true);expect(initial.role).toBe('authority');
 await page.evaluate(()=>m0.wildlife.setProbe(true));expect(await page.evaluate(()=>m0.wildlife.state().debugRoutes)).toBe(12);
 ...
 try{await page.waitForFunction(()=>['takeoff','flying'].includes(m0.wildlife.state().frame.entities.find(e=>e.siteId==='trail-bird-01')?.state),null,{timeout:10000});}catch(error){console.log('BIRD_REACTION',await page.evaluate(()=>({player:m0.state().player,wildlife:m0.wildlife.state()})));throw error;}
 const flight=await page.evaluate(()=>m0.wildlife.state());expect(flight.frame.entities.find(e=>e.siteId==='trail-bird-01').id).toBe(initial.frame.entities.find(e=>e.siteId==='trail-bird-01').id);expect(flight.frame.eventWatermark).toBe(1);
 ...
 expect(await page.evaluate(()=>m0.wildlife.state().frame.entities.find(e=>e.siteId==='trail-bird-01').route)).toEqual(flight.frame.entities.find(e=>e.siteId==='trail-bird-01').route);
