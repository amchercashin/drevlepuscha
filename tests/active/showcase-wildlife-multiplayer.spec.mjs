...
test('W2 shared bird: early guest retained, remote threat outside host render range, atlas and late reload',async({page,browser})=>{
 ...
 try{
  ...
  await page.waitForFunction(()=>m0.wildlife.state().frame.entities.some(e=>e.siteId==='trail-bird-01'));
  const bird=await page.evaluate(()=>m0.wildlife.state().frame.entities.find(e=>e.siteId==='trail-bird-01'));
  ...
  expect(await page.evaluate(()=>m0.wildlife.state().frame.entities.find(e=>e.siteId==='trail-bird-01').id)).toBe(bird.id);
  ...
  await guest.waitForFunction(()=>['takeoff','flying'].includes(m0.wildlife.state().frame.entities.find(e=>e.siteId==='trail-bird-01')?.state),null,{timeout:15000});
  const a=await page.evaluate(()=>m0.wildlife.state()),b=await guest.evaluate(()=>m0.wildlife.state());expect(a.frame.entities.find(e=>e.siteId==='trail-bird-01').id).toBe(b.frame.entities.find(e=>e.siteId==='trail-bird-01').id);expect(a.frame.entities.find(e=>e.siteId==='trail-bird-01').route).toEqual(b.frame.entities.find(e=>e.siteId==='trail-bird-01').route);expect(a.frame.eventWatermark).toBe(1);expect(a.role).toBe('authority');expect(b.role).toBe('replica');
  ...
  for(let i=0;i<5;i++){
   ...
   if(i<4)expect(await client.evaluate(()=>testRoom.wildlife.latest().entities.find(e=>e.siteId==='trail-bird-01').id)).toBe(bird.id);
  }
  expect(await page.evaluate(()=>m0.state().multiplayer.players)).toBe(6);
  await guest.reload({waitUntil:'commit'});await guest.waitForFunction(()=>window.m0?.state().ready,null,{timeout:90000});const late=await guest.evaluate(()=>m0.wildlife.state());expect(late.frame.entities.find(e=>e.siteId==='trail-bird-01').id).toBe(bird.id);expect(late.frame.entities.find(e=>e.siteId==='trail-bird-01').route).toEqual(b.frame.entities.find(e=>e.siteId==='trail-bird-01').route);expect(late.deliveredEvents).toEqual([]);expect(late.frame.entities.find(e=>e.siteId==='trail-bird-01').state).not.toBe('takeoff');
  expect(errors).toEqual([]);
 }catch(error){console.log('WILDLIFE_TEST_ERROR',String(error));console.log('WILDLIFE_HOST',await page.evaluate(()=>({ready:!!window.m0?.state().ready,errors:window.m0?.state().errors,text:document.querySelector('#pause-description')?.textContent})).catch(()=>null));throw error;}finally{for(const client of lightweight){await client.evaluate(()=>testRoom?.leave()).catch(()=>{});await client.close();}await guest?.close();}
});
