...
test('tree tones are bounded, reproducible and continuous between groves',()=>{
 const samples=[];
 for(let i=0;i<200;i++){const tone=treeTone('tree-'+i,i*3-300,i*2-200);assert.deepEqual(tone,treeTone('tree-'+i,i*3-300,i*2-200));tone.forEach((v,j)=>assert.ok(Math.abs(v)<=[.08,.06,.06][j]));samples.push(tone);}
